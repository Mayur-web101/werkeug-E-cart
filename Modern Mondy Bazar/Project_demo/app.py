    ...
    def do_POST(self):
        ...
        #     return self.wfile.write(json.dumps({'cancel': cancel}).encode())
            ...
            for crop_details in details:
                crop_details = {
                        'cropdetail_id': crop_details[0],
                        'c_name': crop_details[2],
                        'detail': crop_details[3],
                        'place': crop_details[4],
                        'qty': crop_details[5],
                        'category': crop_details[6],
                        'state': crop_details[7],
                        }
            ...
            data = self.rfile.read(int(self.headers.get('Content-Length')))
            data = json.loads(data)
            details = self.db_connection.view_stage_details(data)
            return self.wfile.write(json.dumps({'details': details}).encode())
...
def start_server():
    ...
    httpd = HTTPServer(('0.0.0.0', 3100), myHandler)
    httpd.serve_forever()
