...
# INNER JOIN cropdetail ON stage_activity.cropdetail_id=cropdetail.id;
 ...
 # 2 SELECT users.name, users.address, users.mobile_no,crop_book.qty,crop_book.dat,crop_book.status FROM crop_book 
 # INNER JOIN users ON crop_book.u_id=users.id;
